...
exports.EXTENDCIRCUIT = function (circuitId, purpose) {
  return (
    `EXTENDCIRCUIT ${circuitId}` + (purpose ? ` purpose="${purpose}"` : "")
  );
};
...
exports.ATTACHSTREAM = function (streamId, options) {
  return (
    `ATTACHSTREAM ${streamId} ${options.circuitId}` +
...
exports.CLOSECIRCUIT = function (circuitId, options = { ifUnused: false }) {
  return `CLOSECIRCUIT ${circuitId}` + (options.ifUnused ? " IfUnused" : "");
};
...
exports.HSFETCH = function (serviceId, serverLongName) {
  return (
    `HSFETCH ${serviceId}` + (serverLongName ? ` SERVER=${serverLongName}` : "")
  );
};
