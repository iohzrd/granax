...
class TorController extends EventEmitter {
  ...
  static createChallengeResponse(cookie, clientNonce, serverNonce) {
    return crypto
      .createHmac("sha256", TorController.CLIENT_HASH)
      .update(
        Buffer.concat([
  ...
  constructor(socket, options) {
    ...
    this.socket
      .pipe(TorController.createReplySplitter())
  ...
  _authOnConnect(callback) {
    ...
    function sendAuthCommand({ hash, nonce }, cookie, next) {
      ...
      self.authenticate(
        TorController.createChallengeResponse(cookie, clientNonce, nonce, hash),
        next
      );
    }
  ...
  _handleReply(data) {
    ...
    switch (code.toString()[0]) {
      case "2":
        ...
        let parsed = replies[method] ? replies[method](lines) : lines;
  ...
  authenticatePromise(token) {
    return new Promise((resolve, reject) => {
      this._send(commands.AUTHENTICATE(token), (err, data) => {
  ...
  getAuthChallengePromise(nonce) {
    return new Promise((resolve, reject) => {
      this._send(commands.AUTHCHALLENGE(nonce), (err, data) => {
        if (err) return reject(err);
        resolve(data);
      });
    });
  ...
  getProtocolInfoPromise() {
    return new Promise((resolve, reject) => {
      this._send(commands.PROTOCOLINFO(), (err, data) => {
        if (err) return reject(err);
        resolve(data);
      });
    });
  ...
  createHiddenServicePromise(ports, options) {
    if (typeof options === "function") {
      options = {};
    }
    return new Promise((resolve, reject) => {
      this._send(commands.ADD_ONION(ports, options), (err, data) => {
        if (err) return reject(err);
        resolve(data);
      });
    });
  ...
  destroyHiddenServicePromise(serviceId) {
    return new Promise((resolve, reject) => {
      this._send(commands.DEL_ONION(serviceId), (err, data) => {
        if (err) return reject(err);
        resolve(data);
      });
    });
  ...
  setConfigPromise(keyword, value) {
    return new Promise((resolve, reject) => {
      this._send(commands.SETCONF(keyword, value), (err, data) => {
        if (err) return reject(err);
        resolve(data);
      });
    });
  ...
  resetConfigPromise(keyword) {
    return new Promise((resolve, reject) => {
      this._send(commands.RESETCONF(keyword), (err, data) => {
        if (err) return reject(err);
        resolve(data);
      });
    });
  ...
  getConfigPromise(keyword) {
    return new Promise((resolve, reject) => {
      this._send(commands.GETCONF(keyword), (err, data) => {
        if (err) return reject(err);
        resolve(data);
      });
    });
  ...
  saveConfigPromise() {
    return new Promise((resolve, reject) => {
      this._send(commands.SAVECONF(), (err, data) => {
        if (err) return reject(err);
        resolve(data);
      });
    });
  ...
  clearDnsCachePromise() {
    return new Promise((resolve, reject) => {
      this.signal("CLEARDNSCACHE", (err, data) => {
        if (err) return reject(err);
        resolve(data);
      });
    });
  ...
  cleanCircuitsPromise() {
    return new Promise((resolve, reject) => {
      this.signal("NEWNYM", (err, data) => {
        if (err) return reject(err);
        resolve(data);
      });
    });
  ...
  dumpHeartbeatPromise() {
    return new Promise((resolve, reject) => {
      this.signal("HEARTBEAT", (err, data) => {
        if (err) return reject(err);
        resolve(data);
      });
    });
  ...
  signalPromise(sig) {
    return new Promise((resolve, reject) => {
      this._send(commands.SIGNAL(sig), (err, data) => {
        if (err) return reject(err);
        resolve(data);
      });
    });
  ...
  createAddressMappingPromise(target, replacement) {
    return new Promise((resolve, reject) => {
      this._send(commands.MAPADDRESS(target, replacement), (err, data) => {
        if (err) return reject(err);
        resolve(data);
      });
    });
  ...
  createCircuitPromise(purpose) {
    if (typeof purpose === "function") {
      purpose = null;
    }
    return new Promise((resolve, reject) => {
      this._send(commands.EXTENDCIRCUIT("0", purpose), (err, data) => {
        if (err) return reject(err);
        resolve(data);
      });
    });
  ...
  extendCircuitPromise(id) {
    return new Promise((resolve, reject) => {
      this._send(commands.EXTENDCIRCUIT(id), (err, data) => {
        if (err) return reject(err);
        resolve(data);
      });
    });
  ...
  setCircuitPurposePromise(circuitId, purpose) {
    return new Promise((resolve, reject) => {
      this._send(
        commands.SETCIRCUITPURPOSE(circuitId, purpose),
        (err, data) => {
          if (err) return reject(err);
          resolve(data);
        }
  ...
  attachStreamPromise(streamId, options) {
    if (typeof options === "function") {
      options = { circuitId: "0", hopNumber: null };
    }
    return new Promise((resolve, reject) => {
      this._send(commands.ATTACHSTREAM(streamId, options), (err, data) => {
        if (err) return reject(err);
        resolve(data);
      });
    });
  ...
  postDescriptorPromise(descriptor, options) {
    if (typeof options === "function") {
      options = {};
    }
    return new Promise((resolve, reject) => {
      this._send(commands.POSTDESCRIPTOR(descriptor, options), (err, data) => {
        if (err) return reject(err);
        resolve(data);
      });
    });
  ...
  redirectStreamPromise(streamId, address, port) {
    if (typeof port === "function") {
      port = null;
    }
    return new Promise((resolve, reject) => {
      this._send(
        commands.REDIRECTSTREAM(streamId, address, port),
        (err, data) => {
          if (err) return reject(err);
          resolve(data);
        }
  ...
  closeStreamPromise(streamId, reason) {
    if (typeof reason === "function") {
      reason = 1;
    }
    return new Promise((resolve, reject) => {
      this._send(commands.CLOSESTREAM(streamId, reason), (err, data) => {
        if (err) return reject(err);
        resolve(data);
      });
    });
  ...
  closeCicuitPromise(circuitId, options) {
    if (typeof options === "function") {
      options = { ifUnused: false };
    }
    return new Promise((resolve, reject) => {
      this._send(commands.CLOSECIRCUIT(circuitId, options), (err, data) => {
        if (err) return reject(err);
        resolve(data);
      });
    });
  ...
  quitPromise() {
    return new Promise((resolve, reject) => {
      this._send(commands.QUIT(), (err, data) => {
        if (err) return reject(err);
        resolve(data);
      });
    });
  ...
  resolvePromise(address, options) {
    if (typeof options === "function") {
      options = {};
    }
    return new Promise((resolve, reject) => {
      this._send(commands.RESOLVE(address, options.reverse), (err, data) => {
        if (err) return reject(err);
        resolve(data);
      });
    });
  ...
  loadConfigPromise(configText) {
    return new Promise((resolve, reject) => {
      this._send(commands.LOADCONF(configText), (err, data) => {
        if (err) return reject(err);
        resolve(data);
      });
    });
  ...
  takeOwnershipPromise() {
    return new Promise((resolve, reject) => {
      this.takeOwnership((err, data) => {
        if (err) return reject(err);
        resolve(data);
      });
    });
  ...
  dropGuardsPromise() {
    return new Promise((resolve, reject) => {
      this._send(commands.DROPGUARDS(), (err, data) => {
        if (err) return reject(err);
        resolve(data);
      });
    });
  ...
  fetchHiddenServiceDescriptorPromise(serviceId, serverLongName) {
    if (typeof serverLongName === "function") {
      serverLongName = "";
    }
    return new Promise((resolve, reject) => {
      this._send(commands.HSFETCH(serviceId, serverLongName), (err, data) => {
        if (err) return reject(err);
        resolve(data);
      });
    });
  ...
  postHiddenServiceDescriptorPromise(descriptor, serverLongName) {
    if (typeof serverLongName === "function") {
      serverLongName = "";
    }
    return new Promise((resolve, reject) => {
      this._send(commands.HSPOST(descriptor, serverLongName), (err, data) => {
        if (err) return reject(err);
        resolve(data);
      });
    });
  ...
  getInfoPromise(keyword) {
    return new Promise((resolve, reject) => {
      this._send(commands.GETINFO(keyword), (err, data) => {
        if (err) return reject(err);
        resolve(data);
      });
    });
  ...
  addEventListenersPromise(events) {
    return new Promise((resolve, reject) => {
      this._send(commands.SETEVENTS(events), (err, data) => {
        if (err) return reject(err);
        resolve(data);
      });
    });
  ...
  removeEventListenersPromise() {
    return new Promise((resolve, reject) => {
      this._send(commands.SETEVENTS([]), (err, data) => {
        if (err) return reject(err);
        resolve(data);
      });
    });
